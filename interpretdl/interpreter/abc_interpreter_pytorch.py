...
class Interpreter(ABC):
    ...
    def _env_setup(self):
        ...
        self.device = torch.device('cuda:0' if 'gpu' in self.device else 'cpu')    
...
class InputGradientInterpreter(Interpreter):
    ...
    def __init__(self, model: callable, device: str, **kwargs):
        ...
        assert hasattr(model, 'forward'), \
            "model has to be " \
            "an instance of torch.nn.Layer or a compatible one."
        ...
        if self.predict_fn is None or rebuild:
            ...
            def predict_fn(inputs, labels=None):
                ...
                if isinstance(inputs, tuple):
                    ...
                    tensor_inputs = torch.tensor(inputs, device=self.device, requires_grad=True)
                ...
                if gradient_of == 'loss':
                    ...
                    loss = torch.nn.functional.cross_entropy(logits, torch.tensor(labels, device=self.device), reduction='sum')
                else:
                    # logits or probas
                    ...
                    labels_onehot = torch.nn.functional.one_hot(torch.tensor(labels, device=self.device), num_classes=num_classes).float()
...
class InputOutputInterpreter(Interpreter):
    ...
    def __init__(self, model: callable, device: str, **kwargs):
        ...
        """
        Interpreter.__init__(self, model, device, **kwargs)
        #assert hasattr(model, 'forward'), "model has to be an instance of paddle.nn.Layer or a compatible one."
        assert isinstance(model, torch.nn.Module), "model has to be an instance of torch.nn.Module or a compatible one."
