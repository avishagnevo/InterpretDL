...
class FunctionalInformationInterpreter(InputGradientInterpreter):
    ...
    def interpret(self,
        ...
        """The technical details of Functional-Information method are described as follows:
    ...
    def init_corr_mat(inputs, labels, label_class): 
        """
        ...
        The method gets ``dataset_class`` data inputs labeled as this class, and then computes 
        the class's correlation matrix *w.r.t.* those class inputs. 

        Args:
            inputs (str or list): The input image filepath or a list of filepaths or numpy array of read images.
            labels (list or np.ndarray, optional): The target labels to calculate the corr for. The number of labels should be equal 
                to the number of images. If None, calculates the corr overall dataset. Default: ``None``.
            visual (bool, optional): Whether or not to visualize a part of the correlation matrix. Default: ``True``.
            save_path (str, optional): The filepath(s) to save the correlation matrix(s). If None, the correlation matrix will not be 
                saved. Default: ``None``.

        Returns:
            np.ndarray: a dictionary of the correlation matries
        """
